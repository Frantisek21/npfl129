...
def main(args: argparse.Namespace) -> list[float]:
    ...
    for order in range(1, args.range + 1):
        ...
        rmse = np.sqrt(np.mean((prediction - target_test)**2))
        ...
        if args.plot:
            ...
            plt.plot(data_train[:, 0], target_train, "go")
            plt.plot(data_test[:, 0], target_test, "ro")
...
if _name_ == "_main_":
    main_args = parser.parse_args([] if "_file_" not in globals() else None)
