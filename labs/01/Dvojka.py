...
def main(args: argparse.Namespace) -> list[float]:
    ...
    data = pd.DataFrame(data = ys, columns = ["target"])
    ...
    for order in range(1, args.range + 1):
        ...
        data[f'xs_{order}'] = xs ** order

        # TODO: Split the data into a train set and a test set.
        # Use `sklearn.model_selection.train_test_split` method call, passing
        # arguments `test_size=args.test_size, random_state=args.seed`.
        data_train, data_test = sklearn.model_selection.train_test_split(data, test_size = args.test_size, random_state = args.seed)
        ...
        fandaisgay = sklearn.linear_model.LinearRegression()
        fandaisgay.fit(X_train, target_train)

        # TODO: Predict targets on the test set using the `predict` method of the trained model.
        fanduvpindikjemalej = fandaisgay.predict(X_test)
        ...
        if args.plot:
            ...
            plt.plot(np.array(X_train.iloc[:, 0]), np.array(target_train), "go")
            plt.plot(np.array(X_test.iloc[:, 0]), np.array(target_test), "ro")
